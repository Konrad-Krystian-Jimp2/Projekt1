...
v_t add_vert(int vert, int dist){
	v_t t = malloc(sizeof * t);
	t->vert = vert;
	t->dist = dist;
return t;
}
...
static void double_pq_size( pq_t pq ) {
	v_t *nq = realloc( pq->q, 2 * pq->s * sizeof *(pq->q) );
	if( nq == NULL )
		exit( 1 );
	pq->q = nq;
	pq->s *= 2;
...
static void heap_up( v_t h[], int n ) {
...
static void heap_down( v_t h[], int n ) {
